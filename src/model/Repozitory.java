    ...
    public boolean searchWorker(int n) throws SearchWorkerException {
        ...
        while (i.hasNext()) {
            ...
            if (n == w.getId())  { // нашли объект и поместили его в Context.worker_obj
                Context.worker_obj = w;
            return true;
        }
    }
    if (Context.worker_obj==null) { // Если работник не найден, тогда формируем ошибку
 ...
 */
public void drop(Worker w) { this.worker.remove(w);}
...
public boolean searchStatus(int n) throws StatusException {
    ...
    for (Status s : Status.values()) {
        ...
        if (i == n) { // профессия найдена и назначена в Context.worker_status
            Context.worker_status = s;
            return true;
        }
    }
    if (Context.worker_status == null) { // должность не найдена, формируем ошибку.
